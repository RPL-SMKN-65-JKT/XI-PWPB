...
<form enctype="multipart/form-data" method="POST" action="/book-edit/{{$books->slug}}" >
  @csrf
        ...
        <div class="flex flex-col gap-1">
            ...
            <div class="input-box mb-3">
              <label for="imageInput" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Cover Buku</label>
                @if ($books->cover!='')
                <img id="imagePreview" src="{{ asset('storage/cover/'.$books->cover) }}" alt="Preview" style="box-shadow: rgba(189, 189, 212, 0.115) 0px 6px 12px -2px, rgba(0, 0, 0, 0.061) 0px 3px 7px -3px; border-radius: 10px; display: ; border: 1px solid rgba(0, 0, 0, 0.161); max-width: 250px; max-height: 250px;">
                @else 
                ...
                @endif
                <input  type="file" accept="image/*" id="imageInput" name="image" class="pt-2 block w-full border-slate-300 text-sm text-slate-500
        ...
        <div class="flex flex-col gap-1">
          <div class="input-box mb-3">
            <label for="title" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Book Title</label>
                <input value="{{ $books->title}}" name="title" type="text" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Judul Buku" required>
          </div>
          <div class="input-box mb-3">
            <label for="penerbit" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Penerbit</label>
            <input value="{{$books->penerbit}}" name="penerbit" type="text" id="penerbit" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Penerbit Buku" required>
          </div>
          <div class="input-box">
            <label for="message" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Deskripsi</label>
            <textarea id="message"  name="deskripsi" rows="10" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" >{{ $books->deskripsi }}</textarea>
            

          </div>
          <div class="input-box">
            <label for="imageInput" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Ebook Pdf</label>
            <input value="{{$books->pdf}}" type="file" name="pdf" class="pt-2 block w-full border-slate-300 text-sm text-slate-500
                file:mr-4 file:py-2 file:px-4
                file:rounded-full file:border-0
                file:text-sm file:font-semibold
                file:bg-violet-50 file:text-[#287bff]
        ...
        <div class="flex flex-col gap-1">
          <div class="input-box mb-3">
            <label for="penulis" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Penulis</label>
                <input value="{{$books->penulis}}" name="penulis" type="text" id="penulis" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Penulis Buku" required>
          </div>
          <div class="input-box mb-3">
            <label for="halaman" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Halaman</label>
            <input value="{{$books->halaman}}" name="halaman" type="number" id="halaman" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Berapa Halaman Buku" required>
          </div>

          <div class="input-box mb-3">
            <label for="classification" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Klasifikasi Buku</label>
       
            <select name="classification_id" id="classification" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
         
                <option selected disabled>Pilih Klasifikasi Buku</option>
                @foreach ($classification as $klasifikasi)
                    <option value="{{ $klasifikasi->id }}" {{ $klasifikasi->id == $books->classification_id ? 'selected' : '' }}>
                        {{ $klasifikasi->name }}
                    </option>
                @endforeach
            </select>
          ...
          <div class="input-box mb-3">
            ...
            <select value="{{old('type_id')}}" name="type_id" id="type" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
              ...
              <option value="{{$jenis->id}}" {{ $jenis->id == $books->type_id ? 'selected' : '' }}>{{$jenis->name}}</option>
              @endforeach
            </select>
          </div>
          <div class="input-box mb-3">
              <label for="bahasa" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Bahasa</label>
              <input value="{{$books->bahasa}}" name="bahasa" type="text" id="bahasa" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Bahasa Buku" required>
            </div>
