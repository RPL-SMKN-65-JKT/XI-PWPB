...
<form enctype="multipart/form-data" method="POST" action="" >
  @csrf
    <h1 class=" mb-10 text-base font-semibold leading-7 text-gray-900">Profile Admin</h1>
    <div class="grid grid-cols-3 gap-x-4">
        <div class="flex flex-col gap-1">
          <div class="input-box mb-3">
            <label for="imageInput" class="block mb-2 text-sm font-bold text-gray-900 dark:text-white">Profile</label>
              @if (Auth::user()->profile_image!='')
              <img id="imagePreview" src="{{ asset('storage/profile/'.Auth::user()->profile_image) }}" alt="Preview" style="box-shadow: rgba(189, 189, 212, 0.115) 0px 6px 12px -2px, rgba(0, 0, 0, 0.061) 0px 3px 7px -3px; border-radius: 10px; display: ; border: 1px solid rgba(0, 0, 0, 0.161); max-width: 250px; max-height: 250px;">
              @else 
            ...
            <div class="mb-3">
                ...
                <input value="{{ Auth::user()->name}}" name="name" type="text" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="978162537" required>
            </div>
            ...
            <select name="role_id" id="role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                ...
                <option selected disabled>Pilih Role</option>
                 ...
                 @foreach ($role as $posisi)
                    <option value="{{ $posisi->id }}" {{ $posisi->id == Auth::user()->role_id ? 'selected' : '' }}>
                        {{ $posisi->name }}
                    </option>
                @endforeach 
            </select>
