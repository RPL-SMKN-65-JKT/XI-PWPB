...
@section('content')
  <form action="{{ url('/dashboard/users/'.$user->id) }}" method="POST" enctype="multipart/form-data" class="w-full flex flex-col text-black p-4 gap-6" >
    @csrf
    @method('put')
      ...
      <div class="w-full flex flex-col gap-4">
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('name') is-invalid @enderror">
              <input type="text" placeholder="Masukkan Nama Lengkap" class="w-full outline-none" name="name" id="name" required value="{{ old('name', $user->name) }}">
          </div>
          @error('name')
        ...
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('place_of_birth') is-invalid @enderror">
              <input type="text" placeholder="Masukkan Tempat Lahir" class="w-full outline-none" name="place_of_birth" id="place_of_birth" required value="{{ old('place_of_birth', $user->place_of_birth) }}">
          </div>
        ...
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('date_of_birth') is-invalid @enderror">
              <input type="date" placeholder="Masukkan Tanggal Lahir" class="w-full outline-none" name="date_of_birth" id="date_of_birth" required value="{{ old('date_of_birth', $user->date_of_birth) }}">
          </div>
          @error('date_of_birth')
            <span class="invalid-feedback">{{ $message }}</span>
          @enderror
        ...
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('phone_number') is-invalid @enderror">
              <input type="text" placeholder="Masukkan Nomor Telepon" class="w-full outline-none" name="phone_number" id="phone_number" required value="{{ old('phone_number', $user->phone_number) }}">
          </div>
          @error('phone_number')
            <span class="invalid-feedback">{{ $message }}</span>
          @enderror
        ...
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('email') is-invalid @enderror">
              <input type="email" placeholder="Masukkan Nomor Telepon" class="w-full outline-none" name="email" id="email" required value="{{ old('email', $user->email) }}">
          </div>
          @error('email')
            <span class="invalid-feedback">{{ $message }}</span>
          @enderror
      ...
      <div class="w-full flex flex-col gap-4">
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('username') is-invalid @enderror">
              <input type="text" placeholder="Masukkan Username" class="w-full outline-none" name="username" id="username" required value="{{ old('username', $user->username) }}">
          </div>
          @error('username')
            <span class="invalid-feedback">{{ $message }}</span>
          @enderror
        ...
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('password') is-invalid @enderror">
              <input type="password" placeholder="Masukkan Password" class="w-full outline-none" name="password" id="password" required value="{{ old('password', $user->password) }}">
        ...
        <div class="flex flex-col gap-0.5">
          <span class="text-base font-semibold">Alamat</span>
            <textarea name="address" id="address" class="w-full h-[128px] outline-none bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md resize-none" placeholder="Masukkan Alamat">{{ old('address', $user->address) }}</textarea>
            @error('address')
              <span class="invalid-feedback">{{ $message }}</span>
            @enderror
        </div>
        @can('officers')
        <input type="hidden" name="role_id" hidden value="{{ $user->role_id }}">
        @endcan
        @can('admin')  
        <div class="flex flex-col gap-0.5">
