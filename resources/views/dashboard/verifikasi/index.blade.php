...
<form action="/dashboard/verifikasi/found/{{ $bookLoan->kode_peminjaman }}" class="mt-4 text-end" method="POST">
  @csrf
  @method('put')
  <input type="hidden" hidden value="{{ $bookLoan->kode_peminjaman }}">
  <button type="submit" class="bg-orange-primary text-white px-3 py-1 rounded-full">Confirm</button>
</form>
