...
@section('content')
  <form action="{{ url('/dashboard/books') }}" method="POST" enctype="multipart/form-data" class="w-full flex flex-col text-black p-4 gap-6" >
    @csrf
    ...
    <div class="w-full grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div class="w-full flex flex-col gap-4">
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('name') is-invalid @enderror">
              <input type="text" placeholder="Masukkan Judul Buku" class="w-full outline-none" name="title" id="title" required value="{{ old('title') }}">
          </div>
          @error('title')
        ...
        <div class="flex flex-col gap-0.5">
          <span class="text-base font-semibold">Jenis Buku</span>
            <select name="type_id" id="type_id" class="outline-none bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('type_id') is-invalid @enderror" disabled>
        ...
        <div class="flex flex-col gap-1">
          ...
          <div class="w-full grid grid-cols-2 gap-1 bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('categories') is-invalid @enderror" id="category_checkbox_wrapper">
            <span class="text-gray-500">Pilih Genre Buku</span>
          </div>
            @error('type_id')
              <span class="invalid-feedback">{{ $message }}</span>
            @enderror
        ...
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('author') is-invalid @enderror">
              <input type="text" placeholder="Masukkan Penulis Buku" class="w-full outline-none" name="author" required value="{{ old('author') }}">
          </div>
          @error('author')
            <span class="invalid-feedback">{{ $message }}</span>
          @enderror
        ...
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('publisher') is-invalid @enderror">
              <input type="text" placeholder="Masukkan Penerbit Buku" class="w-full outline-none" name="publisher" required value="{{ old('publisher') }}">
          </div>
          @error('publisher')
            <span class="invalid-feedback">{{ $message }}</span>
          @enderror
        ...
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('publication_year') is-invalid @enderror">
              <input type="text" placeholder="Masukkan Tahun Terbit Buku" class="w-full outline-none" name="publication_year" maxlength="4" required value="{{ old('publication_year') }}">
          </div>
        ...
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('isbn') is-invalid @enderror">
              <input type="text" placeholder="Masukkan ISBN Buku" class="w-full outline-none" name="isbn" required value="{{ old('isbn') }}">
          </div>
          @error('isbn')
            <span class="invalid-feedback">{{ $message }}</span>
          @enderror
      ...
      <div class="w-full flex flex-col gap-4">
        <div class="flex flex-col gap-0.5">
          ...
          <div class="flex flex-col gap-2 bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('image') is-invalid @enderror">
            <input type="file" class="w-full outline-none" name="image" id="image" required onchange="previewImage()">
        ...
        <div class="flex flex-col gap-0.5">
          <span class="text-base font-semibold">Deskripsi Buku</span>
            <textarea name="description" id="description" class="w-full h-48 outline-none bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md" placeholder="Masukkan Deskripsi Buku"></textarea>
            @error('description')
              <span class="invalid-feedback">{{ $message }}</span>
            @enderror
        </div>
        <div class="hidden flex flex-col gap-0.5" id="link">
          <span class="text-base font-semibold">Link Ebook (Jika Ada)</span>
          <div class="flex bg-white border border-[#333] px-2.5 py-2 rounded-md shadow-md @error('isbn') is-invalid @enderror">
              <input type="text" placeholder="https://" class="w-full outline-none" name="link_ebook" value="{{ old('link_ebook') }}">
          </div>
          @error('link_ebook')
            <span class="invalid-feedback">{{ $message }}</span>
          @enderror
      ...
      <button type="submit" class="self-end bg-orange-primary text-white py-2 px-8 text-lg font-semibold rounded-full">Upload</button>
    </form>
@endsection
...
@section('script')
  <script>
    ...
    const linkEbook = document.getElementById('link');

    ebook.addEventListener('change', function() {
      if(this.value == 1) {
        linkEbook.classList.remove('hidden');
      }
      else {
        linkEbook.classList.add('hidden');
      }
    })
  </script>
@endsection